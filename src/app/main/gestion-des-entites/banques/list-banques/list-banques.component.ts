...
export class ListBanquesComponent implements OnInit {
  ...
  paginatedBanque: any[] = []; // Subset of data for the current page
  ...
  ngOnInit(): void {
    this.banqueService.getAllBanque().subscribe(
      (data: any) => {
        ...
        this.filteredBanque = [...this.listeBanque];
  ...
  updatePaginatedData(): void {
    ...
    this.paginatedBanque = this.filteredBanque.slice(startIndex, endIndex);
  }
  // Handle page changes
  ...
  applyFilters(): void {
    let filtered = [...this.listeBanque];

    // Filtrage par région
    if (this.selectedRegion) {
      filtered = filtered.filter(banque => banque.idRegion?._id === this.selectedRegion);
    }
    ...
    if (this.searchQuery.trim()) {
      ...
      filtered = filtered.filter(banque =>
        (banque.libelle).toLowerCase().includes(query) ||
        banque.email?.toLowerCase().includes(query)
      );
    }

    // Mettre à jour la liste filtrée et paginée
    this.filteredBanque = filtered;
    this.totalItems = this.filteredBanque.length;
    this.updatePaginatedData();
  }

  selectedRegion: string = ''; // Région sélectionnée
  filteredBanque: any[] = []; // Liste filtrée
  ...
  resetFilters(): void {
    this.selectedRegion = '';
    this.searchQuery = '';
    this.filteredBanque = [...this.listeBanque]; // Réinitialise à la liste complète
    this.totalItems = this.filteredBanque.length;
    this.currentPage = 1;
    this.updatePaginatedData();
  }

  deleteBanque(id: any): void {
    Swal.fire({
      title: "Are you sure?",
