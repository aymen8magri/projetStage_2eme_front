...
export class ListClientComponent implements OnInit {
  ...
  paginatedClient: any[] = []; // Subset of data for the current page
  ...
  ngOnInit(): void {
    this.tiersService.getTiersByType("client").subscribe(
      (data: any) => {
        console.log(data);
        this.listeClient = data;
        this.filteredClient = [...this.listeClient];
  ...
  updatePaginatedData(): void {
    ...
    this.paginatedClient = this.filteredClient.slice(startIndex, endIndex);
  }
  // Handle page changes
  ...
  applyFilters(): void {
    let filtered = [...this.listeClient];

    // Filtrage par banque
    if (this.selectedBanque) {
      filtered = filtered.filter(client => client.idBanque?._id === this.selectedBanque);
    }

    // Filtrage par région
    if (this.selectedRegion) {
      filtered = filtered.filter(client => client.idRegion?._id === this.selectedRegion);
    }
    ...
    if (this.searchQuery.trim()) {
      ...
      filtered = filtered.filter(client =>
        (client.nom + ' ' + client.prenom).toLowerCase().includes(query) ||
  ...
  resetFilters(): void {
    ...
    this.selectedRegion = '';
    this.searchQuery = '';
    this.filteredClient = [...this.listeClient]; // Réinitialise à la liste complète
    this.totalItems = this.filteredClient.length;
    this.currentPage = 1;
    this.updatePaginatedData();
  }
