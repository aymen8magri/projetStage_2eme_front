...
export class ViewClientComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.clientService.getTiersById(this.idClient).subscribe(data => {
      this.client = data;
    });
  }
  ...
  deleteClient() {
    Swal.fire({
      title: "Êtes-vous sûr ?",
      text: "Cette action est irréversible. Voulez-vous vraiment supprimer ce client ?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        this.clientService.deleteTiers(this.idClient).subscribe(
          (data: any) => {
