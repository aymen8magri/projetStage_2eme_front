...
export class ListSocieteComponent implements OnInit {
  ...
  paginatedSociete: any[] = []; // Subset of data for the current page
  ...
  ngOnInit(): void {
    this.societeService.getAllSociete().subscribe(
      (data: any) => {
        ...
        this.filteredSociete = [...this.listeSociete];
  ...
  updatePaginatedData(): void {
    ...
    this.paginatedSociete = this.filteredSociete.slice(startIndex, endIndex);
  }
  // Handle page changes
  ...
  applyFilters(): void {
    let filtered = [...this.listeSociete];

    // Filtrage par banque
    if (this.selectedBanque) {
      filtered = filtered.filter(societe => societe.idBanque?._id === this.selectedBanque);
    }

    // Filtrage par région
    if (this.selectedRegion) {
      filtered = filtered.filter(societe => societe.idRegion?._id === this.selectedRegion);
    }
    ...
    if (this.searchQuery.trim()) {
      ...
      filtered = filtered.filter(societe =>
        (societe.libelle).toLowerCase().includes(query) ||
        societe.email?.toLowerCase().includes(query)
      );
    }

    // Mettre à jour la liste filtrée et paginée
    this.filteredSociete = filtered;
    this.totalItems = this.filteredSociete.length;
    this.updatePaginatedData();
  }

  selectedBanque: string = ''; // Banque sélectionnée
  selectedRegion: string = ''; // Région sélectionnée
  filteredSociete: any[] = []; // Liste filtrée
  ...
  resetFilters(): void {
    ...
    this.selectedRegion = '';
    this.searchQuery = '';
    this.filteredSociete = [...this.listeSociete]; // Réinitialise à la liste complète
    this.totalItems = this.filteredSociete.length;
    this.currentPage = 1;
    this.updatePaginatedData();
  }

  deleteSociete(id: any): void {
    Swal.fire({
      title: "Are you sure?",
