...
@Component({
  selector: 'app-view-societe',
  templateUrl: './view-societe.component.html',
  styleUrls: ['./view-societe.component.scss']
})
export class ViewSocieteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.societeService.getSocieteById(this.idsociete).subscribe(data => {
      this.societe = data;
    });
  }
  ...
  deletesociete() {
    Swal.fire({
      title: "Êtes-vous sûr ?",
      text: "Cette action est irréversible. Voulez-vous vraiment supprimer ce societe ?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        this.societeService.deleteSociete(this.idsociete).subscribe(
          (data: any) => {
            this.router.navigate(['/gestion-des-entities/list-societe']);
            Swal.fire({
              title: "Supprimé !",
              text: "Le societe a été supprimé avec succès.",
              icon: "success",
